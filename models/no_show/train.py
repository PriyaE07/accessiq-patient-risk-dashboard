"""
Train the no-show gradient boosting model end-to-end from raw data.

Usage:
    python train.py

Reproduces the same pipeline built interactively in
notebooks/01_no_show_eda.ipynb and notebooks/03_no_show_baseline_model.ipynb,
...
from preprocess import build_features, clean_no_show_data

# Paths are relative to this file, so the script works regardless of the
# directory it's run from.
THIS_DIR = Path(__file__).resolve().parent
RAW_DATA_PATH = THIS_DIR.parents[1] / "data" / "raw" / "KaggleV2-May-2016.csv"
...
RAW_REFERENCE_THRESHOLD = 0.4

# Superseded 2026-08-14 once the model was calibrated: an expected-value
# analysis (threshold = cost_of_reminder / cost_of_missed_slot, roughly
# $1 / $150) said to flag almost everyone (~0.007) since automated reminders
# are nearly free to send at any scale — but a threshold that low defeats
# the point of *risk-based* flagging, so 0.20 was picked instead as a
# deliberate middle ground (~55% flagged, ~83% recall on the calibrated
...
def main():
    ...
    df_clean = clean_no_show_data(df_raw)
