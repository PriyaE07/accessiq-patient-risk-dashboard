...
## Deployment — ✅ live (2026-08-14)
- **Backend**: Render (free tier) — `https://accessiq-backend-ur89.onrender.com`. Root dir `backend`, build `pip install -r requirements.txt`, start `uvicorn main:app --host 0.0.0.0 --port $PORT`. Secrets: `GEMINI_API_KEY`, `ALLOWED_ORIGINS` (set to the Netlify URL below).
- **Frontend**: Netlify (free tier) — `https://accessiq0814.netlify.app`. Base dir `frontend`, build `npm run build`, publish `dist`. Env: `VITE_API_URL` set to the Render URL above (baked in at build time).
- Code changes that made this possible: `frontend/src/config.js` (API_URL from `VITE_API_URL`, falls back to localhost for dev — every component updated to use it instead of hardcoded URLs), backend CORS reads `ALLOWED_ORIGINS` env var instead of a hardcoded list, `backend/requirements.txt` added, `data/raw/`'s two runtime-needed CSVs un-gitignored and committed (~28MB, public/anonymized, no privacy concern).
- **First-ever git commit** happened as part of this — the repo had a GitHub remote configured but zero commits before this session.
- **Known limitation, accepted not engineered around**: Render free tier has no persistent disk, so `alerts.db` resets on every restart/15-min spin-down. Fine for a portfolio demo; would need a hosted DB (e.g. Postgres) to actually persist long-term.
- **Bug caught and fixed during deployment**: the Gemini client had no request timeout, so a network hiccup between Render and Google's API caused `/explain-chat` to hang 120s+ with no error, surfacing to users as a bare "Load failed." Fixed with an explicit 20s timeout (`HttpOptions(timeout=20000)`) + try/except returning a clean 502 instead of hanging or crashing unhandled.
- **Also fixed during deployment**: Netlify's first deploy attempt didn't apply the configured build settings (`Detected 0 framework(s)`, deployed raw repo files instead of the Vite build) — re-set Base directory/Build command/Publish directory explicitly and triggered a fresh deploy, which then ran `vite build` correctly.

## Status
See `progress-log/` for daily notes. Both models complete and served via FastAPI (now on calibrated scores); React dashboard functional and redesigned with a proper design system, **deployed and live** at the URLs above. **All 6 dashboard feature backlog items are done. The calibration fix is done. Deployment is done and verified end-to-end** (predictions, worklist, Overview, Alerts, and the chatbot all confirmed working against the live backend). No open work remains — the population-mismatch disclosure was deliberately deprioritized (see "Known gap" above).
